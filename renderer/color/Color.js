...
export default class Color
{
    ...
    static buildColor(color)
    {
        ...
        return new Color(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha(), color.isFloat());
    }
    ...
    static buildAlpha(color, alpha)
    {
        ...
        if(color.isFloat())
            if(!(alpha >= 0 && alpha <= 1))
                ...
                return new Color(color.getRed(), color.getGreen(), color.getBlue(), alpha, color.isFloat());
        else
